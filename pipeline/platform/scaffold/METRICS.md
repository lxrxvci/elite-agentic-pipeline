...
- `scripts/dora_collector.py` collects the four DORA metrics from the GitHub API and writes `dora-metrics.prom`.
- `scripts/dora_pushgateway.py` pushes the metrics to a Prometheus Pushgateway.
- The weekly `dora-metrics.yml` workflow runs the collector and pushes metrics to a Pushgateway service.
- View the dashboard locally at `http://localhost:3001/d/dora` (Grafana, admin/admin) or import `observability/dashboards/dora-dashboard.json`.
- Metric names: `elite_deployments_total`, `elite_lead_time_seconds`, `elite_change_failure_rate`, `elite_recovery_time_seconds`.
