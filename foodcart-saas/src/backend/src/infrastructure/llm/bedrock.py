"""Amazon Bedrock adapter for the Foodcart AI Website Assistant.

Uses the Bedrock Runtime ``invoke_model`` API with Anthropic Claude models.
All responses are validated against the ``ChangePreview`` schema before they
are returned. On any failure the adapter degrades to the local deterministic
stub so the UI never hard-fails.
"""
...
def _parse_bedrock_response(payload: dict[str, Any]) -> ChangePreview:
    content_blocks = payload.get("content", [])
    if not content_blocks:
        raise ValueError("Bedrock response contained no content blocks")
    ...
    if not raw_text:
        raise ValueError("Bedrock response content was empty")

    parsed = json.loads(raw_text)
    operations = [PatchOperation(**op) for op in parsed.get("operations", [])]
...
def generate_change_preview_with_bedrock(
    prompt: str,
    ...
    """Call Bedrock and return a validated ``ChangePreview``.
    ...
    client = boto3.client(
        "bedrock-runtime",
        region_name=region,
        config=Config(connect_timeout=_TIMEOUT_SECONDS, read_timeout=_TIMEOUT_SECONDS),
    )
    ...
    try:
        ...
        preview = _parse_bedrock_response(response_payload)
    except Exception as exc:
        logger.warning(
            "bedrock_llm_fallback",
