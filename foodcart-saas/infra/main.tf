...
resource "aws_s3_bucket" "foodcart_uploads" {
  ...
  #checkov:skip=CKV2_AWS_6:Public access is blocked via aws_s3_bucket_public_access_block (count-indexed graph edges are not evaluated by Checkov).
...
resource "aws_kms_key" "foodcart_uploads" {
  ...
  #checkov:skip=CKV2_AWS_64:Key policy is attached via aws_kms_key_policy resource (Checkov does not resolve count-indexed graph edges for this rule).
...
data "aws_iam_policy_document" "kms_uploads" {
  statement {
    ...
    actions   = ["kms:*"]
    resources = [aws_kms_key.foodcart_uploads[0].arn]
  }

  statement {
    sid    = "Allow S3 service for bucket encryption"
    effect = "Allow"
    principals {
      type        = "Service"
...
resource "aws_kms_key_policy" "foodcart_uploads" {
  count = var.enable_storage ? 1 : 0

  key_id = aws_kms_key.foodcart_uploads[0].id
  policy = data.aws_iam_policy_document.kms_uploads.json
}
